...
namespace Dokkaebi.Core.Networking.Commands
{
    ...
    public class MoveCommand : CommandBase
    {
        ...
        public override void Execute()
        {
            ...
            Debug.Log($"[MoveCommand.Execute] Calling unit.MoveToGridPosition({targetGridPos}) for immediate movement...");
            try // Add try-catch for safety
            {
                unit.MoveToGridPosition(targetGridPos);
                Debug.Log("[MoveCommand.Execute] unit.MoveToGridPosition() completed - movement started immediately.");
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"[MoveCommand.Execute] EXCEPTION during unit.MoveToGridPosition(): {ex.Message}\n{ex.StackTrace}");
                return; // Stop execution if movement fails
            }

            // --- Log Final DebugLog ---
            Debug.Log("[MoveCommand.Execute] Calling internal DebugLog...");
            DebugLog($"Started immediate movement for unit {UnitId} to position {TargetPosition}"); // Uses internal DebugLog
